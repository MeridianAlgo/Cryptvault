...
def prepare_features(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for i in range(len(close_prices)):
        if i < 20:
            ...
            # Momentum
            momentum = float((close_prices[i] - close_prices[i-1]) / close_prices[i-1]) if close_prices[i-1] != 0 else 0.0
...
def print_summary(all_results: Dict[str, List[Dict]]):
    ...
    for symbol, results in all_results.items():
        ...
        print(f"\n{symbol}:")
        print(f"{'Model':<25} {'MAPE %':<12} {'Max Err %':<12} {'Within 0.5%':<15} {'Within 1%':<12} {'Dir Acc %':<12}")
        print("-"*120)
