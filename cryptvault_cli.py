...
def analyze_cryptocurrency(ticker: str, days: int, interval: str, verbose: bool = False, 
                          generate_chart: bool = True, save_chart_path: str = None, no_chart: bool = False):
    ...
    # Generate matplotlib chart with pattern overlays (default behavior)
    if generate_chart or save_chart_path:
        try:
            from generate_chart import generate_chart as gen_chart
            
            if save_chart_path:
                print(f"\n[CHART] Generating chart with pattern overlays and saving to {save_chart_path}...")
            else:
                print(f"\n[CHART] Generating interactive chart with pattern overlays...")
                print("[INFO] Chart window will open - use toolbar to zoom/pan, close when done")
            
            gen_chart(ticker, days=days, interval=interval, save_path=save_chart_path)
            
            if save_chart_path:
                print(f"[OK] Chart saved to: {save_chart_path}")
            else:
                print("[OK] Chart window closed")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python cryptvault_cli.py BTC 60 1d                    # Bitcoin with interactive chart
  python cryptvault_cli.py ETH 60 1d --save-chart eth.png  # Save chart to file
  python cryptvault_cli.py ADA 60 1d --no-chart        # Text-only analysis
...
Charts with pattern overlays are generated by default!
    ...
    parser.add_argument('--no-chart', action='store_true',
                       help='Disable automatic chart generation (charts are shown by default)')
    parser.add_argument('--save-chart', metavar='PATH',
                       help='Save chart to file instead of displaying interactively')
    parser.add_argument('--version', action='version', version='CryptVault 3.3.0')
    ...
    try:
        if args.demo:
            ...
            success = analyze_cryptocurrency(
                ...
                args.interval, 
                args.verbose,
                generate_chart=True,  # Always generate by default
