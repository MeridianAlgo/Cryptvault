...
class PortfolioAnalyzer:
    ...
    def analyze_portfolio(self, assets: List[PortfolioAsset], days: int = 30) -> Dict[str, Any]:
            ...
            for asset in assets:
                if asset.symbol in asset_data:
                    ...
                    asset_analysis[asset.symbol] = {
                        ...
                        "top_pattern": (
                            analysis_dict["patterns"][0] if analysis_dict.get("patterns") else None
                        ),
    ...
    def _calculate_portfolio_metrics(
        self, assets: List[PortfolioAsset], asset_data: Dict[str, PriceDataFrame]
    ) -> PortfolioMetrics:
        ...
        for asset in assets:
            if asset.symbol in asset_data and asset.current_price:
                ...
                ) * 10000  # Base portfolio value of $10,000
                portfolio_value += asset_value

        # Get aligned price data
        min_length = min(
            len(asset_data[asset.symbol].data) for asset in assets if asset.symbol in asset_data
        )
        ...
        return PortfolioMetrics(
            ...
            sharpe_ratio=(
                self._calculate_sharpe_ratio(portfolio_returns) if portfolio_returns else 0.0
            ),
            max_drawdown=(
                self._calculate_max_drawdown(portfolio_returns) if portfolio_returns else 0.0
            ),
    ...
    def _generate_portfolio_recommendations(
        ...
        if metrics.max_drawdown < -0.2:
            recommendations.append("High drawdown risk detected - consider adding defensive assets")
    ...
    def suggest_rebalancing(
        self, current_assets: List[PortfolioAsset], target_volatility: float = 0.15
    ) -> Dict[str, float]:
    ...
    def calculate_portfolio_var(
        self, assets: List[PortfolioAsset], confidence: float = 0.95
    ) -> float:
    ...
    def _generate_comparison_insights(
        self, asset_analysis: Dict, correlation_matrix: CorrelationMatrix
    ) -> List[str]:
        ...
        if len(correlation_matrix.symbols) >= 2:
            ...
            if max_pair:
                insights.append(
                    f"Highest correlation: {max_pair[0]}-{max_pair[1]} ({max_corr:.2f})"
                )
            if min_pair:
