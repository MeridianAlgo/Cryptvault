...
class PatternScanner:
    ...
    def _detect_wedges(self, df: pd.DataFrame):
        ...
        df["trend_high"] = (
            ...
            .apply(
                lambda x: (
                    1 if (x.iloc[-1] - x.iloc[0]) > 0 else -1 if (x.iloc[-1] - x.iloc[0]) < 0 else 0
                )
            )
        )
        df["trend_low"] = (
    ...
    def _detect_channels(self, df: pd.DataFrame):
        ...
        mask_up = (
            ...
            & (df["trend_low"] == 1)
            & (
                (df["high_roll_max"] - df["low_roll_min"])
                <= channel_range * (df["high_roll_max"] + df["low_roll_min"]) / 2
            )
        )
    ...
    def _detect_cup_handle(self, df: pd.DataFrame):
        ...
        for i in range(window, len(df)):
            ...
            # Check for U-shape
            if (
                first_third.mean() > middle_third.mean()
                and last_third.mean() > middle_third.mean()
                and abs(first_third.mean() - last_third.mean()) / first_third.mean() < 0.05
            ):
    ...
    def _find_consecutive_groups(
        self, mask: pd.Series, min_length: int = 3
    ) -> List[Tuple[int, int]]:
