...
class CandlestickPatternAnalyzer:
    ...
    def detect_single_candlestick_patterns(
    ...
    def detect_multi_candlestick_patterns(
        self, data: PriceDataFrame, sensitivity: float = 0.5
    ) -> List[DetectedPattern]:
        """
        ...
        for i in range(2, len(data) - 2):  # Need context before and after
            two_candle_patterns = self._analyze_two_candle_patterns(data, i, sensitivity)
        ...
        for i in range(2, len(data) - 3):  # Need one more candle for 3-candle patterns
            three_candle_patterns = self._analyze_three_candle_patterns(data, i, sensitivity)
            patterns.extend(three_candle_patterns)

        return self._filter_overlapping_patterns(patterns)

    def _analyze_two_candle_patterns(
        self, data: PriceDataFrame, index: int, sensitivity: float
    ) -> List[DetectedPattern]:
    ...
    def _analyze_three_candle_patterns(
        self, data: PriceDataFrame, index: int, sensitivity: float
    ) -> List[DetectedPattern]:
        ...
        for detector in pattern_detectors:
            pattern = detector(
                candle1, candle2, candle3, index, stats1, stats2, stats3, context, data, sensitivity
            )
            if pattern:
                patterns.append(pattern)
    ...
    def _detect_bullish_engulfing(
        ...
        if (
            not stats1["is_bullish"]  # First candle bearish
            and stats2["is_bullish"]  # Second candle bullish
            and candle2.open < candle1.close  # Second opens below first close
            and candle2.close > candle1.open  # Second closes above first open
            and stats2["body_ratio"] > 0.5  # Second candle has substantial body
            and context["trend"] in ["downtrend", "sideways"]
        ):  # Appropriate context
    ...
    def _detect_bearish_engulfing(
        ...
        if (
            stats1["is_bullish"]  # First candle bullish
            and not stats2["is_bullish"]  # Second candle bearish
            and candle2.open > candle1.close  # Second opens above first close
            and candle2.close < candle1.open  # Second closes below first open
            and stats2["body_ratio"] > 0.5  # Second candle has substantial body
            and context["trend"] in ["uptrend", "sideways"]
        ):  # Appropriate context
    ...
    def _detect_bullish_harami(
        ...
        if (
            not stats1["is_bullish"]  # First candle bearish
            and stats1["body_ratio"] > 0.6  # First candle large
            and candle2.open < candle1.open  # Second contained within first
            and candle2.close > candle1.close
            and stats2["body_ratio"] < stats1["body_ratio"] * 0.5  # Second much smaller
            and context["trend"] in ["downtrend", "sideways"]
        ):
            ...
            if confidence >= (0.3 + sensitivity * 0.3):
                return self._create_multi_candlestick_pattern(
                    ...
                    confidence,
                    data,
                    "Bullish Harami - potential reversal signal",
                )
    ...
    def _detect_bearish_harami(
        ...
        if (
            stats1["is_bullish"]  # First candle bullish
            and stats1["body_ratio"] > 0.6  # First candle large
            and candle2.open > candle1.close  # Second contained within first
            ...
            if confidence >= (0.3 + sensitivity * 0.3):
                return self._create_multi_candlestick_pattern(
                    ...
                    confidence,
                    data,
                    "Bearish Harami - potential reversal signal",
                )
    ...
    def _detect_piercing_line(
        ...
        if (
            not stats1["is_bullish"]  # First candle bearish
            and stats2["is_bullish"]  # Second candle bullish
            and candle2.open < candle1.low  # Second opens below first's low
            and candle2.close > (candle1.open + candle1.close) / 2  # Closes above midpoint
            and stats1["body_ratio"] > 0.4  # First candle substantial
            and stats2["body_ratio"] > 0.4  # Second candle substantial
            and context["trend"] in ["downtrend", "sideways"]
        ):
            ...
            if confidence >= (0.4 + sensitivity * 0.3):
                return self._create_multi_candlestick_pattern(
                    ...
                    confidence,
                    data,
                    "Piercing Line - bullish reversal signal",
                )
    ...
    def _detect_dark_cloud_cover(
        ...
        if (
            stats1["is_bullish"]  # First candle bullish
            and not stats2["is_bullish"]  # Second candle bearish
            and candle2.open > candle1.high  # Second opens above first's high
            and candle2.close < (candle1.open + candle1.close) / 2  # Closes below midpoint
            and stats1["body_ratio"] > 0.4  # First candle substantial
            and stats2["body_ratio"] > 0.4  # Second candle substantial
            and context["trend"] in ["uptrend", "sideways"]
        ):
    ...
    def _detect_tweezer_tops(
        ...
        if (
            high_diff <= avg_range * 0.02  # Similar highs (within 2% of average range)
            and context["trend"] in ["uptrend", "sideways"]
            and avg_range > 0
        ):  # Valid range
            ...
            if confidence >= (0.3 + sensitivity * 0.3):
                return self._create_multi_candlestick_pattern(
                    PatternType.TWEEZER_TOPS,
                    [candle1, candle2],
                    index,
                    index + 1,
                    confidence,
                    data,
                    "Tweezer Tops - bearish reversal signal",
                )

        return None

    def _detect_tweezer_bottoms(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        index: int,
        ...
        if (
            low_diff <= avg_range * 0.02  # Similar lows (within 2% of average range)
            and context["trend"] in ["downtrend", "sideways"]
            and avg_range > 0
        ):  # Valid range
            ...
            if confidence >= (0.3 + sensitivity * 0.3):
                return self._create_multi_candlestick_pattern(
                    PatternType.TWEEZER_BOTTOMS,
                    [candle1, candle2],
                    index,
                    index + 1,
                    confidence,
                    data,
                    "Tweezer Bottoms - bullish reversal signal",
                )
    ...
    def _detect_morning_star(
        ...
        if (
            not stats1["is_bullish"]  # First bearish
            and stats3["is_bullish"]  # Third bullish
            and stats1["body_ratio"] > 0.5  # First large
            and stats3["body_ratio"] > 0.5  # Third large
            and stats2["body_ratio"] < 0.3  # Second small (star)
            and candle2.high < candle1.close  # Gap down
            and candle3.open > candle2.high  # Gap up
            and candle3.close > (candle1.open + candle1.close) / 2  # Third closes well into first
            and context["trend"] in ["downtrend", "sideways"]
        ):
    ...
    def _detect_evening_star(
        ...
        if (
            stats1["is_bullish"]  # First bullish
            and not stats3["is_bullish"]  # Third bearish
            and stats1["body_ratio"] > 0.5  # First large
            and stats3["body_ratio"] > 0.5  # Third large
            and stats2["body_ratio"] < 0.3  # Second small (star)
            and candle2.low > candle1.close  # Gap up
            and candle3.open < candle2.low  # Gap down
            and candle3.close < (candle1.open + candle1.close) / 2  # Third closes well into first
            and context["trend"] in ["uptrend", "sideways"]
        ):
    ...
    def _detect_three_white_soldiers(
        ...
        if (
            stats1["is_bullish"]
            and stats2["is_bullish"]
            and stats3["is_bullish"]  # All bullish
            and stats1["body_ratio"] > 0.4
            and stats2["body_ratio"] > 0.4
            and stats3["body_ratio"] > 0.4  # All substantial
            and candle2.open > candle1.open
            and candle2.open < candle1.close  # Second opens in first body
            and candle3.open > candle2.open
            and candle3.open < candle2.close  # Third opens in second body
            and candle2.close > candle1.close
            and candle3.close > candle2.close
        ):  # Progressive closes
    ...
    def _detect_three_black_crows(
        ...
        if (
            not stats1["is_bullish"]
            and not stats2["is_bullish"]
            and not stats3["is_bullish"]  # All bearish
            and stats1["body_ratio"] > 0.4
            and stats2["body_ratio"] > 0.4
            and stats3["body_ratio"] > 0.4  # All substantial
            and candle2.open < candle1.open
    ...
    def _detect_rising_three_methods(
        ...
        if (
            stats1["is_bullish"]  # First bullish
            and not stats2["is_bullish"]  # Second bearish (pullback)
            and stats3["is_bullish"]  # Third bullish
            and stats1["body_ratio"] > 0.5  # First large
            and stats3["body_ratio"] > 0.5  # Third large
            and stats2["body_ratio"] < 0.4  # Second smaller
            and candle2.close > candle1.open  # Pullback stays above first open
            and candle3.close > candle1.close  # Third closes higher than first
            and context["trend"] in ["uptrend", "sideways"]
        ):
    ...
    def _detect_falling_three_methods(
        ...
        if (
            not stats1["is_bullish"]  # First bearish
            and stats2["is_bullish"]  # Second bullish (pullback)
            and not stats3["is_bullish"]  # Third bearish
            and stats1["body_ratio"] > 0.5  # First large
            and stats3["body_ratio"] > 0.5  # Third large
            and stats2["body_ratio"] < 0.4  # Second smaller
            and candle2.close < candle1.open  # Pullback stays below first open
            and candle3.close < candle1.close  # Third closes lower than first
            and context["trend"] in ["downtrend", "sideways"]
        ):
    ...
    def _analyze_single_candle(
        ...
        candle: PricePoint,
        index: int,
        prev_candles: List[PricePoint],
        next_candles: List[PricePoint],
        data: PriceDataFrame,
        sensitivity: float,
    ) -> List[DetectedPattern]:
    ...
    def _analyze_market_context(
        self, prev_candles: List[PricePoint], next_candles: List[PricePoint]
    ) -> Dict:
        ...
        return {"trend": trend, "avg_range": avg_range, "prev_closes": prev_closes}
    ...
    def _detect_hammer(
        ...
        if (
            stats["lower_wick_ratio"] >= 0.6  # Long lower wick
            and stats["body_ratio"] <= 0.3  # Small body
            and stats["upper_wick_ratio"] <= 0.1  # Little upper wick
            and context["trend"] in ["downtrend", "sideways"]
        ):  # Appropriate context
    ...
    def _detect_shooting_star(
        ...
        if (
            stats["upper_wick_ratio"] >= 0.6  # Long upper wick
            and stats["body_ratio"] <= 0.3  # Small body
            and stats["lower_wick_ratio"] <= 0.1  # Little lower wick
            and context["trend"] in ["uptrend", "sideways"]
        ):  # Appropriate context
    ...
    def _detect_spinning_top(
        ...
        if (
            self.doji_threshold < stats["body_ratio"] <= 0.3  # Small but visible body
            and stats["upper_wick_ratio"] >= 0.3  # Decent upper wick
            and stats["lower_wick_ratio"] >= 0.3
        ):  # Decent lower wick
    ...
    def _detect_marubozu(
        ...
        if (
            stats["body_ratio"] >= 0.8  # Large body
            and stats["upper_wick_ratio"] <= 0.1  # Little upper wick
            and stats["lower_wick_ratio"] <= 0.1
        ):  # Little lower wick
    ...
    def _detect_gravestone_doji(
        ...
        if (
            stats["body_ratio"] <= self.doji_threshold  # Doji body
            and stats["upper_wick_ratio"] >= 0.7  # Long upper wick
            and stats["lower_wick_ratio"] <= 0.1  # Little lower wick
            and context["trend"] in ["uptrend", "sideways"]
        ):  # Appropriate context
    ...
    def _calculate_hammer_confidence(self, stats: Dict, context: Dict, sensitivity: float) -> float:
        ...
        context_score = (
            ...
            else 0.7 if context["trend"] == "sideways" else 0.3
        )
    ...
    def _calculate_shooting_star_confidence(
        self, stats: Dict, context: Dict, sensitivity: float
    ) -> float:
        ...
        context_score = (
            1.0 if context["trend"] == "uptrend" else 0.7 if context["trend"] == "sideways" else 0.3
        )
        confidence_factors.append(context_score * 0.2)
    ...
    def _calculate_spinning_top_confidence(
        self, stats: Dict, context: Dict, sensitivity: float
    ) -> float:
    ...
    def _calculate_marubozu_confidence(
        self, stats: Dict, context: Dict, sensitivity: float
    ) -> float:
    ...
    def _calculate_gravestone_doji_confidence(
        self, stats: Dict, context: Dict, sensitivity: float
    ) -> float:
    ...
    def _calculate_harami_confidence(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        stats1: Dict,
    ...
    def _calculate_piercing_confidence(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        stats1: Dict,
    ...
    def _calculate_dark_cloud_confidence(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        stats1: Dict,
    ...
    def _calculate_tweezer_confidence(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        stats1: Dict,
    ...
    def _calculate_soldiers_crows_confidence(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        candle3: PricePoint,
        ...
        if bullish:
            progress_score = min(
                1.0, (candle3.close - candle1.open) / max(stats1["total_range"] * 3, 0.001)
            )
        else:
            progress_score = min(
                1.0, (candle1.open - candle3.close) / max(stats1["total_range"] * 3, 0.001)
            )
    ...
    def _calculate_three_methods_confidence(
        self,
        candle1: PricePoint,
        candle2: PricePoint,
        candle3: PricePoint,
        ...
        if rising:
            continuation_score = min(
                1.0, (candle3.close - candle1.close) / max(stats1["body_size"], 0.001)
            )
        else:
            continuation_score = min(
                1.0, (candle1.close - candle3.close) / max(stats1["body_size"], 0.001)
            )
        ...
        context_score = (
            1.0
            if context["trend"] == expected_trend
            else 0.7 if context["trend"] == "sideways" else 0.5
        )
        confidence_factors.append(context_score * 0.2)
    ...
    def _create_multi_candlestick_pattern(
        self,
    ...
    def _create_candlestick_pattern(
        self,
        pattern_type: PatternType,
        candle: PricePoint,
        index: int,
        ...
        volume_profile = VolumeProfile(
            avg_volume=candle.volume, volume_trend="stable", volume_confirmation=candle.volume > 0
        )
    ...
    def _filter_overlapping_patterns(
        ...
        for pattern in patterns:
            ...
            if (
                index not in pattern_by_index
                or pattern.confidence > pattern_by_index[index].confidence
            ):
