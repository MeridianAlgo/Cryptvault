...
class ProductionPredictor:
    ...
    def _initialize_models(self):
        ...
        self.models["hist_gb"] = HistGradientBoostingRegressor(
            max_iter=200, max_depth=8, learning_rate=0.05, l2_regularization=0.1, random_state=42
        )
        ...
        self.models["gb"] = GradientBoostingRegressor(
            n_estimators=150, max_depth=7, learning_rate=0.05, subsample=0.8, random_state=42
        )
        ...
        for name, model in self.models.items():
            try:
                ...
                if X_val is not None and y_val is not None:
                    ...
                    self.model_scores[name] = {"mape": mape, "r2": r2, "score": score}

                    logger.info(f"{name}: MAPE={mape:.3f}%, R²={r2:.4f}")
                else:
                    # No validation, use default score
                    self.model_scores[name] = {"mape": 1.0, "r2": 0.8, "score": 0.8}
    ...
    def predict_multi_step(
        self, X_last: np.ndarray, n_steps: int = 5, feature_updater=None
    ) -> np.ndarray:
        """
