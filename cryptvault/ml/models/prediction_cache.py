...
class PredictionCache:
    ...
    def store_prediction(
        ...
        self.logger.info(
            f"Stored prediction {prediction_id}: {predicted_value} (confidence: {confidence:.1%})"
        )
        ...
        for prediction in self.predictions:
            ...
            if prediction.prediction_type == "price":
                ...
                prediction.accuracy_score = (
                    1.0 - error_percentage if error_percentage <= 1.0 else 0.0
                )
    ...
    def get_accuracy_report(self, days_back: int = 30) -> Dict[str, Any]:
        ...
        recent_predictions = [
            p for p in self.predictions if p.prediction_date >= cutoff_date and p.verified
        ]
        ...
        accurate_count = sum(
            1 for p in recent_predictions if p.accuracy_score and p.accuracy_score >= 0.9
        )
        ...
        # Average error
        avg_error = (
            sum(1 - p.accuracy_score for p in recent_predictions if p.accuracy_score) / total_count
        )
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_prediction": (
                min(p.prediction_date for p in self.predictions) if self.predictions else None
            ),
            "newest_prediction": (
                max(p.prediction_date for p in self.predictions) if self.predictions else None
            ),
        }
