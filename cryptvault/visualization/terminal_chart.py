...
class ColorManager:
    ...
    def get_pattern_color_scheme(self, category: PatternCategory) -> Dict[str, str]:
        ...
        return color_schemes.get(
            category, {"primary": "white", "secondary": "bright_white", "accent": "bg_white"}
        )
...
class TerminalChart:
    ...
    def _plot_price_data(
        self, chart_lines: List[List[str]], data: PriceDataFrame, price_range: Dict
    ):
        ...
        for i, point in enumerate(data.data):
            ...
            for y in range(body_top, body_bottom + 1):
                if 0 <= y < len(chart_lines) and 0 <= x < len(chart_lines[0]):
                    if is_bullish:
                        body_char = (
                            self.symbols["candle_up"]
                            if y == close_y
                            else self.symbols["candle_body"]
                        )
                    else:
                        body_char = (
                            self.symbols["candle_down"]
                            if y == close_y
                            else self.symbols["candle_body"]
                        )
                    ...
                    if self.colors_enabled:
                        body_char = self.color_manager.colorize(
                            body_char, candle_color, bold=(y == close_y)
                        )
    ...
    def _plot_patterns(
        ...
        for pattern in sorted_patterns:
            ...
            start_x = int(
                (pattern.start_index / max(1, data_points - 1)) * (self.chart_area_width - 1)
            )
    ...
    def _draw_pattern_boundaries(
        self, chart_lines: List[List[str]], start_x: int, end_x: int, pattern: DetectedPattern
    ):
    ...
    def _add_pattern_annotation(
        ...
        for y in annotation_positions:
            if (
                0 <= y < len(chart_lines)
                and 0 <= center_x < len(chart_lines[0])
                and chart_lines[y][center_x] == " "
            ):
    ...
    def _draw_support_resistance_lines(
        ...
        if "support_level" in key_levels:
            ...
            self._draw_horizontal_line(
                chart_lines, start_x, end_x, support_y, self.symbols["support_line"], "green"
            )
        ...
        if "resistance_level" in key_levels:
            ...
            self._draw_horizontal_line(
                chart_lines, start_x, end_x, resistance_y, self.symbols["resistance_line"], "red"
            )
    ...
    def _draw_fibonacci_levels(
        ...
        for level_name in key_fib_levels:
            if level_name in pattern.fibonacci_levels:
                ...
                self._draw_horizontal_line(
                    chart_lines, start_x, end_x, level_y, self.symbols["fibonacci_line"], "magenta"
                )

    def _highlight_candlestick_pattern(
        self,
        chart_lines: List[List[str]],
        pattern: DetectedPattern,
        start_x: int,
        ...
        if pattern.start_index == pattern.end_index:
            ...
            for y_offset in [-1, 0, 1]:
                for x_offset in [-1, 0, 1]:
                    ...
                    if (
                        0 <= new_x < self.chart_area_width
                        and 0 <= new_y < self.chart_area_height
                        and chart_lines[new_y][new_x] == " "
                    ):
    ...
    def _draw_diagonal_line(
        ...
        if x1 == x2:  # Vertical line
            for y in range(min(y1, y2), max(y1, y2) + 1):
                if (
                    0 <= y < len(chart_lines)
                    and 0 <= x1 < len(chart_lines[0])
                    and chart_lines[y][x1] == " "
                ):
                    chart_lines[y][x1] = (
                        self.color_manager.colorize("│", color)
                        if self.colors_enabled and color
                        else "│"
                    )
            return
    ...
    def _create_pattern_legend(self, patterns: List[DetectedPattern]) -> str:
        ...
        for i, pattern in enumerate(sorted_patterns[:8]):  # Show top 8 patterns
            ...
            if self.colors_enabled:
                confidence_bar = self.color_manager.colorize(
                    "█" * filled_bars, "green"
            ...
            if self.colors_enabled:
                color_scheme = self.color_manager.get_pattern_color_scheme(pattern.category)
                category_name = self.color_manager.colorize(
                    f"[{category_name}]", color_scheme["secondary"]
                )
            else:
            ...
            # Create pattern line
            pattern_line = (
                f"{i+1:2d}. {symbol} {pattern_name:<25} {category_name:<25} "
        ...
        if self.colors_enabled:
            ...
            confidence_legend = (
                "Confidence: # High (80%+)  + Med-High (60%+)  - Medium (40%+)  . Low (<40%)"
            )
    ...
    def _format_key_levels(self, pattern: DetectedPattern) -> str:
        ...
        if pattern.category == PatternCategory.CANDLESTICK_PATTERN:
            if "pattern_high" in key_levels and "pattern_low" in key_levels:
                info_parts.append(
                    f"Range: {key_levels['pattern_low']:.2f}-{key_levels['pattern_high']:.2f}"
                )
