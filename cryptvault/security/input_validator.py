...
class InputValidator:
    ...
    def __init__(self, strict_mode: bool = True):
        ...
        logger.info(
            f"InputValidator initialized with {len(self.ticker_whitelist)} whitelisted tickers"
        )
    ...
    def validate_ticker(self, ticker: str) -> str:
        ...
        """
        if not ticker:
            raise ValidationError("Ticker symbol cannot be empty", details={"ticker": ticker})
    ...
    def validate_interval(self, interval: str) -> str:
        ...
        if interval not in valid_intervals:
            raise ValidationError(
                ...
                details={"interval": interval, "valid_intervals": sorted(valid_intervals)},
            )

        return interval

    def validate_file_path(
        self, file_path: str, allowed_extensions: Optional[List[str]] = None
    ) -> str:
        """
        ...
        if ".." in file_path:
            raise ValidationError(
                "Directory traversal not allowed", details={"file_path": file_path}
            )

        # Check for absolute paths (security risk)
        if Path(file_path).is_absolute():
            raise ValidationError("Absolute paths not allowed", details={"file_path": file_path})
    ...
    def validate_amount(
        self, amount: float, min_amount: float = 0.0, max_amount: Optional[float] = None
    ) -> float:
        """
        ...
        try:
            ...
            raise ValidationError(
                "Amount must be a valid number", details={"amount": amount, "error": str(e)}
            )
        ...
        if max_amount is not None and amount > max_amount:
            raise ValidationError(
                f"Amount cannot exceed {max_amount}", details={"amount": amount, "max": max_amount}
            )
...
def validate_no_injection(user_input: str, field_name: str = "input") -> None:
    ...
    for pattern in INJECTION_PATTERNS:
        if re.search(pattern, user_input_lower, re.IGNORECASE):
            ...
            raise ValidationError(
                ...
                details={"field": field_name, "reason": "Potential injection attempt"},
            )
    ...
    if found_suspicious:
        ...
        raise ValidationError(
            ...
            details={"field": field_name, "suspicious_chars": found_suspicious},
        )
